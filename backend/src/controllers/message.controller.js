...
export const sendMessage = async (req, res) => {
  try {
    ...
    const { id: receiverId } = req.params; // Extracting receiverId from URL parameters
    ...
    if (image) {
      console.log("Base64 image string received:", image);
      const uploadResponse = await cloudinary.uploader.upload(image, {
        folder: "chat_images", // Optional: Organize uploads into a specific folder
      });
